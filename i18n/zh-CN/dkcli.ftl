...
timezone = 时区
password = 密码
rtc-as-localtime = 是否使用硬件时钟 (RTC) 为基准时间？
swap-size = 虚拟内存文件 (swapfile) 大小 (GiB)
hostname-illage = 指定的主机名中包含无效字符：{ $c }
username-illage = 指定的 UNIX 用户名中包含无效字符：{ $c }
fullname-illage = 指定的用户全名中包含无效字符：':'
